...
export async function requireScrutinyCertificationAccess(request, response, next) {
  try {
    ...
    if (rows.length === 0) {
      ...
      if (adminRows.length > 0) {
        return response.status(403).json({ code: "OFFICIAL_RECORD_EMISSION_FORBIDDEN_FOR_ADMIN" });
      }
