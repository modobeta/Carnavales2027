...
test("una invitación operativa real exige OTP antes de habilitar el rol", {
  ...
  const email = `real-comisario-${randomUUID()}@example.test`;
  await getPool().query(
  ...
  const app = createApp({
    ...
    getSession: async (input) => input.headers.get("x-test-session") === "admin"
      ? { user: { id: adminId, twoFactorEnabled: true } }
      : testAuth.api.getSession(input),
  ...
  try {
    ...
    const invited = await fetch(`${base}/api/v1/users/invitations`, {
      method: "POST",
    ...
    const password = "OperationalPassword-2026!";
    const accepted = await fetch(`${base}/api/v1/invitations/role/accept`, {
      method: "POST",
    ...
    assert.equal((await fetch(`${base}/api/v1/me`, { headers: { cookie } })).status, 403);
    ...
    assert.deepEqual((await me.json()).roles, ["COMISARIO"]);
  } finally {
